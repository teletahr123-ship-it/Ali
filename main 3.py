...
def get_gemini_client():
    """يهيئ عميل Gemini عند الحاجة فقط حتى يبقى البوت قابلاً للتشغيل بدونه."""
...
def results_keyboard(
    ...
    buttons.append([
        InlineKeyboardButton(
            ...
            callback_data=f"ai:{search_id}:{owner_user_id}",
        )
    ])
...
def clean_ai_search_query(text: str) -> str:
    """
    يأخذ من رد Gemini اسم الكتاب فقط، ويزيل أي شرح أو تنسيق زائد.
    لا يسمح للذكاء الاصطناعي بتغيير صيغة أمر البحث أو توسيعها.
    """
    if not text:
        return ""
    cleaned = text.strip().splitlines()[0].strip()
    cleaned = re.sub(r"^```(?:text|plain)?\s*", "", cleaned, flags=re.I)
    cleaned = re.sub(r"\s*```$", "", cleaned).strip()
    cleaned = re.sub(
        r"^(?:اسم\s+الكتاب|عنوان\s+الكتاب|النتيجة|البحث)\s*[:：-]\s*",
        "",
        cleaned,
        flags=re.I,
    )
    cleaned = re.sub(
        r"^\s*بحث\s+(?:المكتبة|مكتبة)\s*(?::|：|-)?\s*",
        "",
        cleaned,
        flags=re.I,
    )
    return normalize_search_query(cleaned)[:200].strip()


async def improve_search_query(query: str) -> str:
    """
    يستخدم Google Search داخل Gemini للتحقق من اسم الكتاب واختيار عنوان
    واحد واضح، ثم يعيد الاسم فقط ليستخدمه Telegram في قنوات المكتبة.
    """
    ...
    prompt = (
        "أنت محرك تحديد اسم كتاب لمكتبة تيليجرام. "
        "استخدم Google Search للتحقق من العبارة ومقارنة النتائج العامة، "
        "ثم اختر عنوان كتاب واحداً هو الأقرب لعبارة المستخدم. "
        "صحح الأخطاء الإملائية والتهجئة والترجمة الصوتية فقط. "
        "حافظ على أسماء الأشخاص والأرقام والجزء المميز من العنوان. "
        "لا تختر كتاباً مختلفاً لمجرد أنه أشهر. إذا كانت العبارة واضحة "
        "فأعد العنوان القياسي لها، وإذا لم تستطع تحديد كتاب واحد بثقة "
        "فأعد عبارة المستخدم بعد تنظيفها. "
        "لا تضف اسم مؤلف أو كلمات وصفية من عندك، ولا تشرح، ولا تكتب "
        "أمر بحث. أعد سطراً واحداً يحتوي اسم الكتاب فقط.\n\n"
        f"اسم البحث: {query}"
    )
    ...
    improved = clean_ai_search_query(getattr(response, "text", ""))
    if not improved:
        raise ValueError("Gemini returned an empty search name")
    return improved
...
async def handle_search(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    results = await search_with_fallback(query, data["channels"])
    ...
    if not results:
        await msg.edit_text(
            f"لم يتم العثور على نتائج لـ: {html.escape(query)}",
            parse_mode="HTML",
...
async def cb_improve_search(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await q.answer("جاري تحسين اسم البحث...")
    try:
        improved_query = await improve_search_query(query)
    except RuntimeError:
        logger.error("AI search requested but GEMINI_API_KEY is missing")
        await q.message.reply_text(
            "ميزة تحسين البحث غير مفعلة حالياً. أضف GEMINI_API_KEY إلى Secrets ثم أعد تشغيل البوت."
        )
        return
    except Exception as error:
        logger.error(f"AI search improvement error: {error}")
        await q.message.reply_text(
            "تعذر تحسين اسم البحث الآن. يمكنك إعادة المحاولة أو البحث بالاسم الأصلي."
        )
        return

    if improved_query == query:
        await q.message.reply_text(
            f"✅ اسم البحث صحيح بالفعل:\n{html.escape(query)}",
            parse_mode="HTML",
        )
        return
    ...
    await q.message.edit_text(
        f"جاري البحث بالاسم المصحح: {html.escape(improved_query)}...",
        parse_mode="HTML",
    )
    results = await search_with_fallback(improved_query, data["channels"])
    save_search_session(
        search_id,
        improved_query,
        results,
    )
    # تحفظ الجلسة السابقة داخل الوثيقة حتى يظهر للمستخدم ما الذي تم تصحيحه.
    _searches_collection.update_one(
        {"_id": search_id},
    ...
    if not results:
        await q.message.edit_text(
            f"لم يتم العثور على نتائج للاسم المصحح:\n"
            f"{html.escape(improved_query)}\n\n"
            f"الاسم الأصلي: {html.escape(query)}",
            parse_mode="HTML",
            reply_markup=results_keyboard(
                results,
                0,
...
def main() -> None:
    ...
    app.add_handler(CallbackQueryHandler(cb_improve_search, pattern="^ai:"))
