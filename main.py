...
def get_gemini_model():
    """يهيئ نموذج Gemini عند الحاجة فقط حتى يبقى البوت قابلاً للتشغيل بدونه."""
...
async def improve_search_query(query: str) -> str:
    """
    يصحح أخطاء اسم الكتاب فقط. لا يطلب من Gemini البحث أو اقتراح كتب
    بديلة، لأن البحث الفعلي يجب أن يبقى داخل قنوات المكتبة.
    """
    ...
    prompt = (
        "أنت مصحح أسماء كتب لمكتبة تيليجرام. "
        "صحح الأخطاء الإملائية والتهجئة والترجمة الصوتية في اسم الكتاب "
        "فقط، مع الحفاظ على نية المستخدم وأسماء الأشخاص والمؤلفين. "
        "لا تضف كلمات وصفية، ولا تشرح، ولا تقترح كتاباً آخر، ولا تكتب "
        "أمر بحث. إذا كان الاسم صحيحاً فأعده كما هو. "
        "أعد سطراً واحداً يحتوي اسم الكتاب فقط.\n\n"
        f"اسم البحث: {query}"
    )
